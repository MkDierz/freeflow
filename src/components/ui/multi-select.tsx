...
function transToGroupOption(options: Option[], groupBy?: string) {
  ...
  options.forEach((option) => {
    ...
    const selected = groupOption[key];
    if (selected) {
  ...
  (
        ...
        if (input) {
          if (e.key === "Delete" ?? e.key === "Backspace") {
            if (input.value === "" && selected.length > 0) {
              const opt = selected[selected.length - 1];
              if (opt) {
                handleUnselect(opt);
              }
            }
    ...
    return (
      ...
      >
        <div
          className={cn(
            "group cursor-pointer rounded-md border border-input px-3 py-2 text-sm ring-offset-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2",
            className,
          )}
        >
          <div className="flex cursor-pointer flex-wrap gap-1">
            ...
            <CommandPrimitive.Input
              ...
              className={cn(
                "ml-2 flex-1 cursor-pointer bg-transparent outline-none placeholder:text-muted-foreground",
